...
def menor_rating(datos):
    menor = datos[datos["Rating Count"] == datos["Rating Count"].min()].Title
    return menor

def mayor_rating(datos):
    mayor = datos[datos["Rating Count"] == datos["Rating Count"].max()].Title
    return mayor
"""
la película con mayor rating (‘Rating Count’) de 2023
"""
def mayor_rating_2023(datos):
    filtro_por_año = datos[datos["Release Year"] == 2023.0]
...
def año_mayor_rating_drama(datos):
    ...
    año = genero_drama[genero_drama["Rating Count"] == genero_drama["Rating Count"].max()]["Release Year"]
    return año
"""
...
def dataframe(datos,directores):
    ...
    return filtro.loc[:,["Title","Director","Rating Count"]].copy(deep=True)
...
def correlacion(datos):
    datos.plot.scatter(x="Release Year", y="Rating Count", logy=True)
    pend, orig, r, p, err = st.linregress(datos["Release Year"], datos["Rating Count"])
    ...
    pearson = corrs.loc["Release Year","Rating Count"]
...
def comparacion_duracion_2003_2004_normalizacion(datos):
    ...
    # Calcula la media y la desviación estándar de la columna Rating Count
    ...
    # Calcular el promedio de Rating Count Normalizado para cada director
...
def comparacion_entre_directores(datos):
    media_por_ratings=datos[["Director","Rating Count"]].groupby("Director").mean()
    datos[datos["Director"].isin(["Jesús Franco","Lesley Selander"])].boxplot("Rating Count",by="Director")
    ...
    promedios = datos.groupby('Director')['Rating Count'].mean()
...
def comparacion_entre_directores_normalizado(datos):
    ...
    # Calcular la media y la desviación estándar de la columna Rating Count
    mean_rating = df_directores['Rating Count'].mean()
    std_rating = df_directores['Rating Count'].std()
    # Aplicar la normalización (z-score)
    df_directores = df_directores.copy()
    df_directores['Rating Count Normalizado'] = (df_directores['Rating Count'] - mean_rating) / std_rating
    # Calcular el promedio de Rating Count Normalizado para cada director
    promedios_normalizados = df_directores.groupby('Director')['Rating Count Normalizado'].mean()
    # Mostrar los promedios normalizados
    print(promedios_normalizados)
    
    media_por_ratings= df_directores[["Director","Rating Count Normalizado"]].groupby("Director").mean()
    df_directores[df_directores["Director"].isin(["Jesús Franco","Lesley Selander"])].boxplot("Rating Count Normalizado",by="Director")
    plt.plot([1,2],media_por_ratings.loc[["Jesús Franco","Lesley Selander"]],"o")
    #Achico el rango del grafio
    plt.ylim(-1,4)
    plt.title("Comparación entre Ratings de directores")
